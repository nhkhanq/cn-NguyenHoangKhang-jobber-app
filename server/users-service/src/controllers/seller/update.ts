...
import { updateSeller } from '@users/service/seller.service'
import { BadRequestError, ISellerDocument } from 'jobber-shared-for-hkhanq'
...
const seller = async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(StatusCodes.OK).json({ message: 'Seller update successfully.', seller: updatedSeller })
}
