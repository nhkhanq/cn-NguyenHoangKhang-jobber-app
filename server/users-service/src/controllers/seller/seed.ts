...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from 'jobber-shared-for-hkhanq'
import { floor, random, sample, sampleSize } from 'lodash'
import { Request, Response } from 'express'
import { getRandomBuyers } from '@users/service/buyer.service'
import { createSeller, getSellerByEmail } from '@users/service/seller.service'
...
const seed = async (req: Request, res: Response): Promise<void> => {
  ...
  for(let i = 0; i < buyers.length; i++) {
    ...
    const seller: ISellerDocument = {
      ...
      socialLinks: ['https://www.linkedin.com', 'http://youtube.com', 'https://facebook.com'],
      certificates: [
        {
...
const randomExperiences = (count: number): IExperience[] => {
  ...
  for(let i = 0; i < count; i++) {
    const randomStartYear = [2024, 2025, 2026, 2027, 2028, 2029]
...
const randomEducation = (count: number): IEducation[] => {
  ...
  for(let i = 0; i < count; i++) {
    const randomYear = [2024, 2025, 2026, 2027, 2028, 2029]
    const education = {
      country: faker.location.country(),
